...
function CheckTable(props: { tableData: RowObj[] }) {
  ...
  const columns = [
    ...
    columnHelper.accessor('email', {
      id: 'email',
      header: () => <p className='text-xs font-semibold text-zinc-500 dark:text-zinc-400'>EMAIL ADDRESS</p>,
      cell: (info) => <p className='text-sm font-medium text-zinc-950 dark:text-white'>{info.getValue()}</p>
    }),
    columnHelper.accessor('provider', {
      id: 'provider',
      header: () => <p className='text-xs font-semibold text-zinc-500 dark:text-zinc-400'>PROVIDER</p>,
      cell: (info) => (
    ...
    columnHelper.accessor('created', {
      id: 'created',
      header: () => <p className='text-xs font-semibold text-zinc-500 dark:text-zinc-400'>CREATED</p>,
      cell: (info) => (
        <div className='flex w-full items-center gap-[14px]'>
          <p className='text-sm font-medium text-zinc-950 dark:text-white'>{info.getValue()}</p>
        </div>
    ...
    columnHelper.accessor('lastsigned', {
      id: 'lastsigned',
      header: () => <p className='text-xs font-semibold text-zinc-500 dark:text-zinc-400'>LAST SIGN IN</p>,
      cell: (info) => <p className='text-sm font-medium text-zinc-950 dark:text-white'>{info.getValue()}</p>
    }),
    columnHelper.accessor('uuid', {
      id: 'uuid',
      header: () => <p className='text-xs font-semibold text-zinc-500 dark:text-zinc-400'>USER UID</p>,
      cell: (info) => <p className='text-sm font-medium text-zinc-950 dark:text-white'>{info.getValue()}</p>
    }),
      ...
      <div className='overflow-x-scroll xl:overflow-x-hidden'>
        <Table className='w-full'>
          {table.getHeaderGroups().map((headerGroup) => (
            <TableHeader key={headerGroup.id} className='border-b-[1px] border-zinc-200 p-6 dark:border-zinc-800'>
        ...
        <div className='mt-2 flex h-20 w-full items-center justify-between px-6'>
          ...
          <div className='flex items-center gap-3'>
            <p className='text-sm font-medium text-zinc-500 dark:text-zinc-400'>Showing 6 rows per page</p>
          </div>
          {/* right side */}
