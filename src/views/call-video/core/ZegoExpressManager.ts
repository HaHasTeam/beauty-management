...
export class ZegoExpressManager {
  ...
  private deviceUpdateCallback: ((updateType: ZegoDeviceUpdateType, userID: string, roomID: string) => void)[] = []
  ...
  checkWebRTC(): Promise<boolean> {
    return ZegoExpressManager.engine.checkSystemRequirements('webRTC').then((data: ZegoCapabilityDetection) => {
      return !!data.result
    })
  }
  checkCamera(): Promise<boolean> {
    return ZegoExpressManager.engine.checkSystemRequirements('camera').then((data: ZegoCapabilityDetection) => {
      return !!data.result
    })
  }
  checkMicrophone(): Promise<boolean> {
    return ZegoExpressManager.engine.checkSystemRequirements('microphone').then((data: ZegoCapabilityDetection) => {
      return !!data.result
    })
  }
  joinRoom(roomID: string, token: string, user: ZegoUser, options?: ZegoMediaOptions[]): Promise<boolean> {
    ...
    return ZegoExpressManager.engine.loginRoom(roomID, token, user, config).then(async (result) => {
      if (result) {
        this.localParticipant.camera = this.mediaOptions.includes(ZegoMediaOptions.PublishLocalVideo)
        this.localParticipant.mic = this.mediaOptions.includes(ZegoMediaOptions.PublishLocalAudio)
        if (this.localParticipant.camera || this.localParticipant.mic) {
          ...
          const localStream = await ZegoExpressManager.engine.createStream(source)
          this.streamMap.set(this.localParticipant.streamID, localStream)
          // Determine if srcObject has been updated
          if (this.localParticipant.renderView && !this.localParticipant.renderView.srcObject) {
            this.localParticipant.renderView.srcObject = localStream
          }
          await ZegoExpressManager.engine.startPublishingStream(this.localParticipant.streamID, localStream)
        }
      }
      return result
    })
  ...
  enableCamera(enable: boolean): boolean {
    ...
    const result = ZegoExpressManager.engine.mutePublishStreamVideo(streamObj, !enable)
  ...
  enableMic(enable: boolean): boolean {
    const streamID = this.localParticipant.streamID
    const streamObj = this.streamMap.get(streamID) as MediaStream
    const result = ZegoExpressManager.engine.mutePublishStreamAudio(streamObj, !enable)
  ...
  leaveRoom() {
    ...
    this.roomID = ''
    this.localParticipant.renderView && (this.localParticipant.renderView.srcObject = null)
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
  ...
  onRoomUserUpdate(fun: (updateType: 'DELETE' | 'ADD', userList: string[], roomID: string) => void) {
  ...
  onRoomUserDeviceUpdate(fun: (updateType: ZegoDeviceUpdateType, userID: string, roomID: string) => void) {
  ...
  private async playStream(stream: ZegoStreamList) {
    if (
      ...
      const remoteStream = await ZegoExpressManager.engine.startPlayingStream(stream.streamID, playOption)
  ...
  private generateVideoView(type: ZegoVideoViewType, userID: string): HTMLMediaElement {
  ...
  private onOtherEvent() {
        ...
        streamList.forEach((stream) => {
          ...
          if (updateType === 'ADD') {
            ...
            if (participant) {
              ...
              // inner roomStreamUpdate -> inner roomUserUpdate -> out roomUserUpdate
              this.participantDic.set(stream.user.userID, participant_ as ZegoParticipant)
              this.streamDic.set(stream.streamID, participant_ as ZegoParticipant)
            }
    ...
    ZegoExpressManager.engine.on('publishQualityUpdate', (streamID: string, stats: ZegoPublishStats) => {
    ...
    ZegoExpressManager.engine.on('playQualityUpdate', (streamID: string, stats: ZegoPlayStats) => {
    ...
    ZegoExpressManager.engine.on('remoteCameraStatusUpdate', (streamID: string, status: 'OPEN' | 'MUTE') => {
      const participant = this.streamDic.get(streamID)
      if (participant) {
        const updateType = status === 'OPEN' ? ZegoDeviceUpdateType.CameraOpen : ZegoDeviceUpdateType.CameraClose
    ...
    ZegoExpressManager.engine.on('remoteMicStatusUpdate', (streamID: string, status: 'OPEN' | 'MUTE') => {
      const participant = this.streamDic.get(streamID)
      if (participant) {
        const updateType = status === 'OPEN' ? ZegoDeviceUpdateType.MicUnmute : ZegoDeviceUpdateType.MicMute
